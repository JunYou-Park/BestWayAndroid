...
class AccountViewModel(
    ...
    // 로그인 관련 코드   =====
    private val _signInState = MutableLiveData<Resource<DataResult>>()
    val signInState: LiveData<Resource<DataResult>> get() = _signInState
    ...
    fun signIn(email:String, password:String){
        ...
        if(netWorkState.isNetworkConnected()){ // network connected
            ...
            addToDisposable(accountRepository.signIn(email, password).ioNewThread()
                .subscribeWith(object: DisposableObserver<DataResult>(){
                    ...
                    override fun onComplete() {
                        when(result.status){
                            // 로그인 성공한 상태
                            "201" -> {
                                _signInState.postValue(Resource.success(result))
                            }
                            // 기존에 로그인한 유저가 있는 상태
                            "202" ->{
                                _signInState.postValue(Resource.success(result))
                            }
                            else -> _signInState.postValue(Resource.error(result.msg.toString(), null))
                        }
                    }
                }))
        }
    ...
    // 회원가입
    fun signUp(fullName: String, email: String, password: String, phoneNumber: String,createAt: String){
        var result = DataResult()
        showLog(createAt, "createAt")
        _signUpState.postValue(Resource.loading(null))
        if(netWorkState.isNetworkConnected()){
            addToDisposable(accountRepository.singUp(fullName, email, password, phoneNumber, createAt).ioNewThread()
                .subscribeWith(object : DisposableObserver<DataResult>() {
                    override fun onNext(t: DataResult) {
                        showLog(t, "signUp")
                        result = t
