...
class HomeFrag : BaseFrag() {
    ...
    private fun setOnClickListener(){
        binding.lineHomeMealTicketContainer.setOnClickListener {
            (requireActivity() as MainAct).showLoginDialog { state, token ->
                when(state){
                    // 로그인 성공
                    "success" -> getTicket()

                    // 로그인이 다른 곳에서 되어있는 경우
                    "disconnect"-> (requireActivity() as MainAct).showSessionDialog(token!!) { if (it) getTicket() else showMsg(getString(R.string.fail_update_session)) }
                }
            }
        }
    }
