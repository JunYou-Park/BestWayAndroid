...
class SignInDialog() : BaseBottomSheetDialog() {
    ...
    private fun setOnClickListener(){
        binding.btnSignInLogin.setOnClickListener {
            signInViewModel.signIn(binding.tieSignInId.text.toString(), binding.tieSignInPw.text.toString())
        }
    ...
    private fun setObserver(){
        ...
        signInViewModel.signInState.observe(viewLifecycleOwner, Observer{
            ...
            when(resource.status){
                Status.SUCCESS ->{
                    // 로그인 성공
                    var token: TokenVo? = null
                    val gson = Gson()
                    val data = resource.data?.data
                    if(data!=null){
                        for(obj in data){
                            token = gson.fromJson(obj, TokenVo::class.java)
                        }
                        showLog(token.toString(), "signIn token: ")

                    }
                    if(resource.data?.status == "201"){
                        signInListener?.onLogin(token!!)
                        dismiss()
                    }
                    // 기존에 로그인되어 있는 경우
                    else if(resource.data?.status == "202"){
                        // 기존의 로그인 세션을 끊을건지 물어봄
                        signInListener?.onDisConnection(token!!)
                        dismiss()
                    }
                }
                Status.ERROR ->{
